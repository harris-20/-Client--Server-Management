...
// The function recursively searches for a file with the provided filename aldo within a given directory. It also collects information about the file found,file size,file permissions , modification time and complete path
...
// Function to send a redirection message to a client, informing it about a mirror port number
void mirrorRedirection(int clientFileDescriptor) 
{
    char redirectingMessage[1024]; // to store the redirection message.
    snprintf(redirectingMessage, 1024, "%d\n", mirror_p); // Construct a message containing the port number for redirection
    send(clientFileDescriptor, redirectingMessage, strlen(redirectingMessage), 0); // Send the redirection message to the client
    close(clientFileDescriptor);  // Close the client file descriptor
}
...
int main(int argc, char const* argv[]) 
{
    ...
    int activeClients = 0; //To track of the number of active client connections.
   
   const char *dirPath = "/home/raj/Documents/MAC/ASPProject/f23project";
    ...
    address.sin_port = htons(server_p); //To set the port number of the server
    ...
    while (1)  //Endless Loop 
    {
        ...
        if (activeClients <= 3 || (activeClients > 7 && activeClients % 2)) // Load balancing from server to mirror
        { 
        ...
        {
            printf("Redirecting to Mirror Server\n");  // Redirection message to mirror server
            
            mirrorRedirection(newSocket); // Redirect client to mirror server
        }
        activeClients++; // Count for number of client connections
    }
